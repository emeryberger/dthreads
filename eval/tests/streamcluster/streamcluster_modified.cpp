...
// LTP: don't need parec_barrier since GREASE will provide the support for that.
#endif
...
// LTP:  We call actually pthread_***_init to initialize it to use grease 
#endif
...
#ifdef ENABLE_THREADS
    pthread_cond_broadcast(&cond);
//	fprintf(stderr, "%d : send out broadcast, open true\n", getpid());
    pthread_mutex_unlock(&mutex);
#endif
  }
#ifdef ENABLE_THREADS
...
//  fprintf(stderr, "%d -pid %d : stride %d cl %x, Ki2-k1 %d\n", getpid(), pid, stride, cl, k2-k1);

  if( pid==0 )    { 
    work_mem = (double*) malloc(stride*(nproc+1)*sizeof(double));
  //	fprintf(stderr, "pid %d : stride %d allocation now with work_mem %p size %d\n", pid, stride, work_mem, stride*(nproc+1)*sizeof(double));
    gl_cost_of_opening_x = 0;
    gl_number_of_centers_to_close = 0;
  }
  ...
  for ( i = k1; i < k2; i++ ) {
    ...
    if ( x_cost < current_cost ) {
      ...
      // or else dist(p[i], p[x]) would be 0)
  	  ...
  	  //fprintf(stderr, "%d : after barrier to modify %p to 1\n", getpid(), &switch_membership[i]);
      switch_membership[i] = 1;
  ...
  if( pid == 0 )
    {
      ...
      for( int i = 0; i< points->num; i++ ) {
//		fprintf(stderr, "%d - %d: try to modify is_center %d at %p\n", getpid(), i, points->p[i].assign, &is_center[points->p[i].assign]);
	is_center[points->p[i].assign]= true;
      }
    }
		    ...
		    long chunksize, long centersize, char* outfile )
{
  ...
  static long kfinal;
  // long kfinal;
  // LTP: we cannot use stack variable here.
  while(1) {
    ...
    fprintf(stderr,"switch_membership = 0x%08x, is_center = 0x%08x\n", (int)switch_membership, (int)is_center);
  ...
  //  fprintf(stderr,"finish local search\n");
    ...
    if( kfinal + centers.num > centersize ) {
	  ...
	  fprintf(stderr, "kfinal %d, centers.num %d, centersize %d\n", kfinal, centers.num, centersize);
      exit(1);
    }
...
// Added by Tongping Liu
  ...
  fprintf(stderr, "kmin %d kmax %d dim %d chunksize %d clustersize %d\n", kmin, kmax, dim, chunksize, clustersize);
